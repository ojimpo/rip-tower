...
export default function JobDetail() {
  ...
  const deleteArtworkMutation = useMutation({
    mutationFn: (artworkId: number) => api.deleteArtwork(jobId, artworkId),
    onSuccess: invalidateJob,
  });
        ...
        <div className="px-4 pt-4">
          ...
          {artworks.filter((a) => a.selected).map((a) => (
            <div key={a.id} className="mb-4">
              <div className="relative rounded-xl border-2 border-[#e94560]/50 bg-[#16213e] overflow-hidden">
                <button
                  type="button"
                  onClick={() => {
                    if (window.confirm("このアートワークを削除しますか？")) {
                      deleteArtworkMutation.mutate(a.id);
                    }
                  }}
                  disabled={deleteArtworkMutation.isPending}
                  className="absolute top-2 right-2 z-10 w-8 h-8 rounded-full bg-black/60 hover:bg-black/80 text-white flex items-center justify-center transition disabled:opacity-50"
          ...
          {artworks.filter((a) => !a.selected).length > 0 && (
            <div className="grid grid-cols-2 gap-3 mb-4">
              {artworks.filter((a) => !a.selected).map((a) => (
                <div
                  key={a.id}
                  className="relative rounded-xl bg-[#16213e] border border-white/5 overflow-hidden hover:border-white/15 transition"
                >
                  <button
                    type="button"
                    onClick={(e) => {
                    ...
                    disabled={deleteArtworkMutation.isPending}
                    className="absolute top-1.5 right-1.5 z-10 w-7 h-7 rounded-full bg-black/60 hover:bg-black/80 text-white flex items-center justify-center transition disabled:opacity-50"
                    aria-label="アートワークを削除"
                  >
